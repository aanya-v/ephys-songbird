function [syllable_spects,labels_all,raw_audio,syl_idx] = getSpects(syllables,params)
...
syl_idx = []; %chronological syllable trial index
...
for i = 1:length(recmat)
    ...
    if exist(fn_notmat)
        ...
        for x=1:length(labels_select)
            ...
            [~,~,~,P1] = spect_from_waveform(syl_wav,Fs,0,spect_params); %get syllable spectrogram
            ...
            % syllable_spects = [syllable_spects; P_collapsed]; 

            % concatenate the uncut no of windows per syllable
            win_lengths = [win_lengths; size(P1,2)];
        end
    end
end
...
xlabel('bin lengths (ms)')
syl_min_win = labels_all(ind_min)

syllable_spects=flattenArray(P); % saves original spectrograms generated
raw_audio=flattenArray(A); % saves raw audio vectors
